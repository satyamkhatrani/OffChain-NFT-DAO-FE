...
const WithdrawModal = (props) => {
  ...
  const withdrawERC20 = async () => {
    ...
    await treasuryContract.withdrawToken(amt);
    setShowModal(false);
  };

  const withdrawNFT = async () => {
    await treasuryContract.withdrawNFT(depositAmt);
    setShowModal(false);
  };

  const withdrawETH = async () => {
    const amt = ethers.utils.parseEther(depositAmt);
    let tx = await treasuryContract.withdrawEth(amt);
    console.log(tx);
    setShowModal(false);
  };
    ...
    <div className="flex justify-center items-center overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
      <div className="relative w-auto my-6 mx-auto max-w-3xl">
        {loading ? (
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-gray-800 outline-none focus:outline-none">
            <div className="flex items-start justify-between p-5  border-gray-300 rounded-t self-center">
              <span className="text-xl text-white">{`Withdraw ${name}`}</span>
            </div>
                ...
                <div className="px-6 pt-6 pb-8" style={{ width: 450 }}>
                  <div className="w-full mb-2">
                    ...
                    <input
                      ...
                      placeholder={
                        data.name === "NFT"
                          ? "tokenId that you want to Withdraw"
